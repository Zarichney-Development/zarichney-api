#!/bin/bash
# Validate command markdown YAML frontmatter against JSON schema
# Usage: ./validate-commands-frontmatter.sh [file1.md file2.md ...] (optional - validates all if not provided)
...
readonly SCHEMA_FILE="$ROOT_DIR/Docs/Templates/schemas/command-definition.schema.json"
...
log_info() {
    echo -e "${BLUE}[CMD-VALIDATE]${NC} $1"
}

log_success() {
    echo -e "${GREEN}[CMD-VALIDATE]${NC} $1"
}

log_error() {
    echo -e "${RED}[CMD-VALIDATE]${NC} $1"
}

log_warning() {
    echo -e "${YELLOW}[CMD-VALIDATE]${NC} $1"
}
...
# Find all command markdown files (excluding README.md)
find_all_commands() {
    find "$ROOT_DIR/.claude/commands" -name "*.md" -type f ! -name "README.md" 2>/dev/null | sort
}

# Main validation logic
main() {
    log_info "🔍 Command Frontmatter Validation"
    echo ""
    ...
    if [[ $# -gt 0 ]]; then
        ...
        while IFS= read -r file; do
            files_to_validate+=("$file")
        done < <(find_all_commands)

        if [[ ${#files_to_validate[@]} -eq 0 ]]; then
            log_warning "No command files found in $ROOT_DIR/.claude/commands"
            return 0
        fi

        log_info "Validating all ${#files_to_validate[@]} command file(s)"
    fi
    ...
    if [[ $INVALID_FILES -gt 0 ]]; then
        ...
        log_info "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        echo ""
        log_error "❌ Command frontmatter validation FAILED"
        log_info "Fix validation errors or consult schema: $SCHEMA_FILE"
        return 1
    else
        log_info "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        echo ""
        log_success "✅ All command frontmatter validation PASSED"
        return 0
    fi
}
