...
public class AuditorPromptValidationTests : IntegrationTestBase
{
  ...
  /// <summary>
  /// Gets the repository root directory by traversing upward to find .git folder.
  ...
  private static string GetRepositoryRoot()
  {
    ...
    if (dir == null)
      throw new InvalidOperationException("Could not find repository root (no .git folder found)");

    return dir.FullName;
  }

  /// <summary>
  /// Gets the path to the iterative coverage auditor prompt file using dynamic repository root discovery.
  /// This replaces the hard-coded absolute path to ensure cross-platform compatibility.
  /// </summary>
  private static string GetAuditorPromptPath()
  {
    var repositoryRoot = GetRepositoryRoot();
    return Path.Combine(repositoryRoot, ".github", "prompts", "iterative-coverage-auditor.md");
  }
  ...
  public void ValidateAuditorPromptContainsRequiredAiFrameworkVariables()
  {
    ...
    var promptPath = GetAuditorPromptPath();
