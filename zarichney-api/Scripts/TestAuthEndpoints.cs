...
public class AuthResponse
{
  ...
  public string? UserId { get; init; } // May not be present in actual API
}
...
public class TestAuthEndpoints
{
  ...
  private const string TestEmail = "placeholder";
  ...
  private static async Task TestEmailConfirmation()
  {
    ...
    if (string.IsNullOrEmpty(_userId))
    {
      ...
      Console.WriteLine(
        "Note: UserId was not returned in registration. In a real scenario, you'd extract it from the confirmation URL in the email.");
      Console.ResetColor();
    ...
    var resendResult = await MakeRequest<AuthResponse, ResendConfirmationRequest>("resend-confirmation",
  ...
  private static async Task TestRevokeToken()
  {
    ...
    {
      var request = new HttpRequestMessage(HttpMethod.Post, $"{BaseUrl}/revoke");
      ...
      Console.WriteLine($"\n=== Testing: POST {BaseUrl}/revoke (with auth) ===\n");
  ...
  private static async Task TestPasswordReset()
  {
    ...
    var forgotResult = await MakeRequest<AuthResponse, ForgotPasswordRequest>("email-forgot-password",
    ...
    await MakeRequest<AuthResponse, ForgotPasswordRequest>("email-forgot-password", new ForgotPasswordRequest
    {
      Email = "nonexistent@example.com"
    });
