    ...
    ClientConfig clientConfig) : IRequestHandler<ForgotPasswordCommand, AuthResult>
{
    ...
    public async Task<AuthResult> Handle(ForgotPasswordCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            // For development, include the token in the result, but it won't be shown to the user in production
            // In production, we would return only the message
#if DEBUG
            return AuthResult.Ok("Password reset link has been sent to your email", null, null, null, null);
#else
