...
* **`ApiClientFixture`:** Provides a pre-configured instance of the auto-generated Refit client (`IZarichneyAPI`), using an `HttpClient` obtained from the `CustomWebApplicationFactory`.
...
* **Refit Client (`IZarichneyAPI`):** The auto-generated, type-safe HTTP client used for all API interactions in integration tests. Generated by `Scripts/generate-api-client.ps1`.
...
All interactions with the `api-server` under test **must** go through the `IZarichneyAPI` Refit client.

* **Accessing the Client:** The `ApiClient` property (from `IntegrationTestBase`) provides an instance of `IZarichneyAPI`. For authenticated requests, use helper methods like `AuthHelper.CreateAuthenticatedApiClientAsync(...)` (see Section 6.3).
* **Making API Calls:**
    ```csharp
    // Example: GET request
