...
* **Configuration Requirements & Feature Availability:**
   * A strongly-typed `Feature` enum defines all features available in the application that can be enabled or disabled based on configuration availability (e.g., `OpenAiApi`, `MsGraph`, etc.). [cite: api-server/Config/FeatureEnum.cs]
