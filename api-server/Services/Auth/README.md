...
* **Security Notes:** JWT Secret Key is critical; must be kept confidential. Refresh tokens are stored in the DB but should be treated as sensitive. API Keys are stored hashed/validated securely. Password reset and email confirmation tokens have limited validity. `setup-admin` endpoint is potentially insecure and should be reviewed/removed post-setup. [cite: api-server/Controllers/AuthController.cs] HttpOnly cookies mitigate XSS risk for tokens.
...
* **Setup:** Requires a running PostgreSQL instance. Configure `ConnectionStrings:IdentityConnection` and `JwtSettings` (especially `SecretKey`) via user secrets or environment variables for local development. Run EF Core migrations (`dotnet ef database update --context UserDbContext`). [cite: Docs/PostgreSqlMaintenance.md]
